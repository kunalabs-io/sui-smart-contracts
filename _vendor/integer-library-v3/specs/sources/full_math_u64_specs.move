module integer_library_specs::full_math_u64_specs;
...
public fun full_mul_spec(num1: u64, num2: u64): u128 {
...
public fun mul_div_floor_spec(num1: u64, num2: u64, denom: u64): u64 {
    ...
    asserts(expected_result_int.lte(std::u64::max_value!().to_int()));
    ...
    ensures(product_int.lt(result_int.add(1u64.to_int()).mul(denom_int)));
...
public fun mul_div_round_spec(num1: u64, num2: u64, denom: u64): u64 {
    asserts(denom > 0);
    let product_int = num1.to_int().mul(num2.to_int());
    let half_denom_int = denom.to_int().div(2u64.to_int());
...
public fun mul_div_ceil_spec(num1: u64, num2: u64, denom: u64): u64 {
    asserts(denom > 0);
    let product_int = num1.to_int().mul(num2.to_int());
    let denom_int = denom.to_int();
    let expected_result_int = product_int.add(denom_int.sub(1u64.to_int())).div(denom_int);
    asserts(expected_result_int.lte(std::u64::max_value!().to_int()));
    ...
    let result_int = result.to_int();
    ensures(result_int.sub(1u64.to_int()).mul(denom_int).lt(product_int));
...
public fun mul_shr_spec(num1: u64, num2: u64, shift: u8): u64 {
    asserts(shift <= 127);
    let product_int = num1.to_int().mul(num2.to_int());
    let power_of_two = 2u64.to_int().pow(shift.to_int());
...
public fun mul_shl_spec(num1: u64, num2: u64, shift: u8): u64 {
    asserts(shift <= 127);
    let product_int = num1.to_int().mul(num2.to_int());
    let expected_result = product_int
        .shl(shift.to_int())
        .mod(std::u128::max_value!().to_int().add(1u64.to_int()));
    asserts(expected_result.lte(std::u64::max_value!().to_int()));
