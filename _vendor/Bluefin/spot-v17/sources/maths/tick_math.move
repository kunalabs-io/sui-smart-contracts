...
module bluefin_spot::tick_math {
    ...
    public fun max_tick(): I32 {
        i32H::lib_to_mate(i32::from(TICK_BOUND))
    }

    public fun min_tick(): I32 {
        i32H::lib_to_mate(i32::neg_from(TICK_BOUND))
    }
    ...
    public fun get_sqrt_price_at_tick(tick: I32): u128 {
        assert!(i32H::gte(tick, min_tick()) && i32H::lte(tick, max_tick()), EINVALID_TICK);
        if (i32H::is_neg(tick)) {
    ...
    public fun is_valid_index(index: I32, tick_spacing: u32): bool {
        let in_range = i32H::gte(index, min_tick()) && i32H::lte(index, max_tick());
        in_range && (i32::mod(i32H::mate_to_lib(index), i32::from(tick_spacing)) == i32::from(0))
    }
    ...
    public fun get_tick_at_sqrt_price(sqrt_price: u128): I32 {
        ...
        i32H::lib_to_mate(
            if (i32::eq(tick_low, tick_high)) {
                tick_low
            } else if (get_sqrt_price_at_tick(i32H::lib_to_mate(tick_high)) <= sqrt_price) {
                tick_high
            } else {
                tick_low
            })
    ...
    fun get_sqrt_price_at_negative_tick(tick: I32): u128 {
        let abs_tick = i32::as_u32(i32::abs(i32H::mate_to_lib(tick)));
    ...
    fun test_get_sqrt_price_at_tick() {
        // min tick
        assert!(get_sqrt_price_at_tick(i32H::lib_to_mate(i32::neg_from(TICK_BOUND))) == 4295048016u128, 2);
        // max tick
        assert!(get_sqrt_price_at_tick(i32H::lib_to_mate(i32::from(TICK_BOUND))) == 79226673515401279992447579055u128, 1);
        assert!(get_sqrt_price_at_tick(i32H::lib_to_mate(i32::neg_from(435444u32))) == 6469134034u128, 3);
        assert!(get_sqrt_price_at_tick(i32H::lib_to_mate(i32::from(408332u32))) == 13561044167458152057771544136u128, 4);
    }

    #[test]
    fun test_get_tick_at_sqrt_price_1() {
        assert!(i32::eq(i32H::mate_to_lib(get_tick_at_sqrt_price(6469134034u128)), i32::neg_from(435444)) == true, 0);
        assert!(i32::eq(i32H::mate_to_lib(get_tick_at_sqrt_price(13561044167458152057771544136u128)), i32::from(408332u32)) == true, 0);
    }

    #[test]
    #[expected_failure]
    fun test_get_sqrt_price_at_invalid_upper_tick() {
        get_sqrt_price_at_tick(i32H::lib_to_mate(i32::add(i32H::mate_to_lib(max_tick()), i32::from(1))));
    }

    #[test]
    #[expected_failure]
    fun test_get_sqrt_price_at_invalid_lower_tick() {
        get_sqrt_price_at_tick(i32H::lib_to_mate(i32::sub(i32H::mate_to_lib(min_tick()), i32::from(1))));
    }
