...
module move_stl::skip_list_u128_tests {
    use move_stl::skip_list_u128::{
        SkipList,
        insert,
        remove,
        find_prev,
    ...
    use move_stl::random;
    use move_stl::option_u128::{is_some, is_none};
    ...
    fun add_node_for_test<V: store + copy + drop>(list: &mut SkipList<V>, size: u64, seed: u64, value: V) {
        ...
        let mut n = 0;
        while (n < size) {
            let score = random.rand_n(1000000) as u128; 
    ...
    fun test_find() {
        let ctx = &mut tx_context::dummy();
        let list = new_list_for_test<u256>(16, 2, 20, 12345, 0, ctx);
