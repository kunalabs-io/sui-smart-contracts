module kai_leverage::position_core_clmm {
    ...
    use fun turbos_clmm::math_tick::sqrt_price_from_tick_index as turbos_clmm::i32::I32.as_sqrt_price_x64;
    /*
    use fun kai_leverage::turbos::calc_deposit_amounts_by_liquidity as turbos_clmm::pool::Pool.calc_deposit_amounts_by_liquidity;
    use fun kai_leverage::turbos::wrapped_tick_range as kai_leverage::turbos::TurbosWrappedPosition.tick_range;
    use fun kai_leverage::turbos::wrapped_liquidity as kai_leverage::turbos::TurbosWrappedPosition.liquidity;
    */
    ...
    use fun kai_leverage::cetus::calc_deposit_amounts_by_liquidity as cetus_clmm::pool::Pool.calc_deposit_amounts_by_liquidity;
    ...
    /*
    use fun kai_leverage::flowx::calc_deposit_amounts_by_liquidity as flowx_clmm::pool::Pool.calc_deposit_amounts_by_liquidity;
    ...
    */
    ...
    public(package) fun position_constructor<X, Y, LP>(
        config_id: ID, lp_position: LP, col_x: Balance<X>, col_y: Balance<Y>,
        debt_bag: FacilDebtBag, collected_fees: BalanceBag, owner_reward_stash: BalanceBag,
        ctx: &mut TxContext
    ): Position<X, Y, LP> {
        Position {
            id: object::new(ctx),
    ...
    public(package) fun position_debt_bag_mut<X, Y, LP>(position: &mut Position<X, Y, LP>): &mut FacilDebtBag {
        &mut position.debt_bag
    }

    public(package) fun collected_fees_mut<X, Y, LP>(position: &mut Position<X, Y, LP>): &mut BalanceBag {
    ...
    public struct PositionConfig has key {
        ...
        /// When the position margin level is below or at this value, it can be deleveraged. Multiplied by 10000.
        deleverage_margin_bps: u16,
        base_deleverage_factor_bps: u16,
        /// When the position margin level is below or at this value, it can be liquidated. Multiplied by 10000.
        liq_margin_bps: u16,
    ...
    public fun set_allow_new_positions(
        config: &mut PositionConfig, value: bool, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public fun set_min_liq_start_price_delta_bps(
        config: &mut PositionConfig, value: u16, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public fun config_add_empty_pyth_config(
        config: &mut PositionConfig, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public fun set_pyth_config_max_age_secs(
        config: &mut PositionConfig, max_age_secs: u64, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public fun pyth_config_allow_pio(
        config: &mut PositionConfig, coin_type: TypeName, pio_id: ID, ctx: &mut TxContext
    ): ActionRequest {
        let pyth_config: &mut PythConfig = &mut config.allowed_oracles[type_name::get<PythConfig>()];
    ...
    public fun pyth_config_disallow_pio(
        config: &mut PositionConfig, coin_type: TypeName, ctx: &mut TxContext
    ): ActionRequest {
        let pyth_config: &mut PythConfig = &mut config.allowed_oracles[type_name::get<PythConfig>()];
    ...
    public fun set_max_position_l(
        config: &mut PositionConfig, value: u128, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public fun set_position_creation_fee_sui(
        config: &mut PositionConfig, value: u64, ctx: &mut TxContext
    ): ActionRequest {
    ...
    public(package) fun deleverage_ticket_constructor(
        position_id: ID, can_repay_x: bool, can_repay_y: bool, info: DeleverageInfo
    ): DeleverageTicket {
    ...
    public(package) fun reduction_repayment_ticket_constructor<SX, SY>(
        sx: FacilDebtShare<SX>, sy: FacilDebtShare<SY>, info: ReductionInfo
    ...
    public(package) fun collected_amm_rewards_mut(self: &mut RebalanceReceipt): &mut VecMap<TypeName, u64> {
    ...
    public(package) fun new_create_position_ticket<X, Y, I32>(
        config_id: ID, tick_a: I32, tick_b: I32, dx: u64, dy: u64, delta_l: u128,
        principal_x: Balance<X>, principal_y: Balance<Y>, borrowed_x: Balance<X>, borrowed_y: Balance<Y>,
        debt_bag: FacilDebtBag
    ...
    public(package) fun destroy_create_position_ticket<X, Y, I32>(
        ...
        let CreatePositionTicket {
            config_id, tick_a, tick_b, dx, dy, delta_l, principal_x, principal_y,
            borrowed_x, borrowed_y, debt_bag
         } = ticket;

         (config_id, tick_a, tick_b, dx, dy, delta_l, principal_x, principal_y, borrowed_x, borrowed_y, debt_bag)
    }
    ...
    public(package) fun borrowed_x_mut<X, Y, I32>(ticket: &mut CreatePositionTicket<X, Y, I32>): &mut Balance<X> {
    ...
    public(package) fun borrowed_y_mut<X, Y, I32>(ticket: &mut CreatePositionTicket<X, Y, I32>): &mut Balance<Y> {
    ...
    public(package) fun cpt_debt_bag<X, Y, I32>(ticket: &CreatePositionTicket<X, Y, I32>): &FacilDebtBag {
        &ticket.debt_bag
    }

    public(package) fun cpt_debt_bag_mut<X, Y, I32>(ticket: &mut CreatePositionTicket<X, Y, I32>): &mut FacilDebtBag {
        &mut ticket.debt_bag
    }
    ...
    public(package) fun emit_position_creation_info(
        position_id: ID, config_id: ID, sqrt_pa_x64: u128, sqrt_pb_x64: u128, l: u128,
        x0: u64, y0: u64, cx: u64, cy: u64, dx: u64, dy: u64, creation_fee_amt_sui: u64
    ) {
    ...
    public(package) fun deleverage_info_constructor(
        position_id: ID, model: PositionModel, oracle_price_x128: u256, sqrt_pool_price_x64: u128,
        delta_l: u128, delta_x: u64, delta_y: u64, x_repaid: u64, y_repaid: u64
    ): DeleverageInfo {
        DeleverageInfo {
            position_id,
            model,
    ...
    public(package) fun emit_liquidation_info(
        position_id: ID, model: PositionModel, oracle_price_x128: u256,
        x_repaid: u64, y_repaid: u64, liquidator_reward_x: u64, liquidator_reward_y: u64,
        liquidation_fee_x: u64, liquidation_fee_y: u64
    ) {
    ...
    public(package) fun reduction_info_constructor(
        position_id: ID, model: PositionModel, oracle_price_x128: u256, sqrt_pool_price_x64: u128,
        delta_l: u128, delta_x: u64, delta_y: u64, withdrawn_x: u64, withdrawn_y: u64, x_repaid: u64, y_repaid: u64
    ): ReductionInfo {
        ReductionInfo {
            position_id,
            model,
    ...
    public(package) fun add_liquidity_info_constructor(
        position_id: ID, sqrt_pool_price_x64: u128, delta_l: u128, delta_x: u64, delta_y: u64
    ): AddLiquidityInfo {
        AddLiquidityInfo {
            position_id,
            sqrt_pool_price_x64,
    ...
    public(package) fun check_versions<X, Y, LP>(position: &Position<X, Y, LP>, config: &PositionConfig) {
    ...
    public fun migrate_position<X, Y, LP>(position: &mut Position<X, Y, LP>, ctx: &mut TxContext): ActionRequest {
    ...
    public(package) fun validate_price_info(
        config: &PositionConfig, price_info: &PythPriceInfo
    ...
    public(package) fun validate_debt_info(config: &PositionConfig, debt_info: &DebtInfo): ValidatedDebtInfo {
    ...
    public(package) fun liq_margin_is_valid(
        config: &PositionConfig, model: &PositionModel, p0_min_x128: u256, p0_max_x128: u256
    ): bool {
    ...
    public(package) macro fun model_from_position<$X, $Y, $LP>(
        $position: &Position<$X, $Y, $LP>, $debt_info: &ValidatedDebtInfo
    ): PositionModel {
        ...
        position_model_clmm::create(
            sqrt_pa_x64, sqrt_pb_x64, l, cx, cy, dx, dy
        )
    }

    public(package) macro fun slippage_tolerance_assertion(
        $pool_object: _, $p0_desired_x128: u256, $max_slippage_bps: u16
    ) {
    ...
    public(package) macro fun create_position_ticket<$X, $Y, $I32>(
        ...
        let p0_oracle_x128 = price_info.div_price_numeric_x128(type_name::get<$X>(), type_name::get<$Y>());
        ...
        let model = {
            ...
            position_model_clmm::create(
                sqrt_pa_x64, sqrt_pb_x64, $delta_l, cx, cy, dx, dy
            )
        };
        ...
        let debt_bag = supply_pool::empty_facil_debt_bag(
            object::id(config.lend_facil_cap()), $ctx
        );
        new_create_position_ticket(
            config_id, tick_a, tick_b, model.dx(), model.dy(), $delta_l,
            principal_x, principal_y, balance::zero(), balance::zero(), debt_bag
        )
    }
    ...
    public(package) macro fun create_position<$X, $Y, $I32, $Pool, $LP>(
        ...
        let lp_position = $open_position(
            pool_object, tick_a, tick_b, delta_l, balance_x0, balance_y0
        );
        ...
        let position = position_constructor(
            object::id(config), lp_position, col_x, col_y, debt_bag, collected_fees, owner_reward_stash, $ctx
        );
    ...
    public(package) macro fun create_deleverage_ticket_inner<$X, $Y, $Pool, $LP>(
        ...
        let mut info = {
            ...
            deleverage_info_constructor(
                position_id, model, oracle_price_x128, sqrt_pool_price_x64,
                delta_l, delta_x, delta_y, x_repaid, y_repaid,
            )
        };
        ...
        if (!model.margin_below_threshold(p_x128, threshold_margin)) {
            ...
            let ticket = deleverage_ticket_constructor(object::id(position), can_repay_x, can_repay_y, info);
            return ticket
        };
        ...
        let delta_l = util::min_u128(
            ...
            model.calc_max_deleverage_delta_l(
                p_x128, config.deleverage_margin_bps(), config.base_deleverage_factor_bps()
            )
        );
    ...
    public(package) macro fun create_deleverage_ticket<$X, $Y, $Pool, $LP>(
        ...
        let ticket = create_deleverage_ticket_inner!(
            $position, $config, $price_info, $debt_info, $pool_object,
            $max_delta_l, false, $remove_liquidity
        );
    ...
    public(package) macro fun create_deleverage_ticket_for_liquidation<$X, $Y, $Pool, $LP>(
        ...
        create_deleverage_ticket_inner!(
            $position, $config, $price_info, $debt_info, $pool_object, u128_max, true, $remove_liquidity
        )
    }
    ...
    public fun destroy_deleverage_ticket<X, Y, LP: store>(
        position: &mut Position<X, Y, LP>, ticket: DeleverageTicket
    ) {
        assert!(ticket.position_id == object::id(position), EPositionMismatch);
    ...
    public(package) macro fun deleverage<$X, $Y, $SX, $SY, $Pool, $LP>(
        ...
        let (mut ticket, request) = create_deleverage_ticket!(
            position, config, $price_info, &debt_info, $pool_object, $max_delta_l, $ctx, $remove_liquidity
        );
    ...
    public(package) macro fun deleverage_for_liquidation<$X, $Y, $SX, $SY, $Pool, $LP>(
        ...
        let mut ticket = create_deleverage_ticket_for_liquidation!(
            position, config, $price_info, &debt_info, $pool_object, $remove_liquidity
        );
        deleverage_ticket_repay_x(position, config, &mut ticket, supply_pool_x, $clock);
        deleverage_ticket_repay_y(position, config, &mut ticket, supply_pool_y, $clock);
        destroy_deleverage_ticket(position, ticket);
    ...
    public(package) fun calc_liq_fee_from_reward(config: &PositionConfig, reward_amt: u64): u64 {
        util::muldiv(
            reward_amt, (config.liq_bonus_bps as u64) * (config.liq_fee_bps as u64),
    ...
    public(package) macro fun liquidate_col_x<$X, $Y, $SY, $LP>(
        ...
        let (repayment_amt_y, reward_amt_x) = model.calc_liquidate_col_x(
            p_x128, repayment.value(), config.liq_margin_bps(), config.liq_bonus_bps(), config.base_liq_factor_bps(),
        );
...

        {
            ...
            emit_liquidation_info(
                position_id, model, oracle_price_x128, x_repaid, y_repaid,
                liquidator_reward_x, liquidator_reward_y, liquidation_fee_x, liquidation_fee_y
            );
        };
    ...
    public(package) macro fun reduce<$X, $Y, $SX, $SY, $Pool, $LP>(
        ...
        let oracle_price_x128 = price_info.div_price_numeric_x128(type_name::get<$X>(), type_name::get<$Y>());
        let sqrt_pool_price_x64 = pool_object.current_sqrt_price_x64();
        let pool_price_x128 = 
            (sqrt_pool_price_x64 as u256) * (sqrt_pool_price_x64 as u256);
        ...
        let (mut got_x, mut got_y) = $remove_liquidity(pool_object, position.lp_position_mut(), delta_l);
        ...
        let info = {
            ...
            reduction_info_constructor(
                object::id(position), model, oracle_price_x128, sqrt_pool_price_x64,
                delta_l, delta_x, delta_y, withdrawn_x, withdrawn_y, x_repaid, y_repaid
            )
        };
    ...
    public fun reduction_ticket_calc_repay_amt_x<X, SX, SY>(
        ticket: &ReductionRepaymentTicket<SX, SY>, supply_pool: &mut SupplyPool<X, SX>, clock: &Clock
    ): u64 {
    ...
    public fun reduction_ticket_calc_repay_amt_y<Y, SX, SY>(
        ticket: &ReductionRepaymentTicket<SX, SY>, supply_pool: &mut SupplyPool<Y, SY>, clock: &Clock
    ): u64 {
    ...
    public fun reduction_ticket_repay_x<X, SX, SY>(
        ...
        supply_pool: &mut SupplyPool<X, SX>,balance: Balance<X>, clock: &Clock
    ) {
    ...
    public fun reduction_ticket_repay_y<Y, SX, SY>(
        ticket: &mut ReductionRepaymentTicket<SX, SY>,
        supply_pool: &mut SupplyPool<Y, SY>, balance: Balance<Y>, clock: &Clock
    ) {
    ...
    public fun add_collateral_x<X, Y, LP: store>(
        position: &mut Position<X, Y, LP>, cap: &PositionCap, balance: Balance<X>
    ) {
        check_position_version(position);
        assert!(cap.position_id == object::id(position), EInvalidPositionCap);
    ...
    public fun add_collateral_y<X, Y, LP: store>(
        position: &mut Position<X, Y, LP>, cap: &PositionCap, balance: Balance<Y>
    ) {
        check_position_version(position);
        assert!(cap.position_id == object::id(position), EInvalidPositionCap);
    ...
    public(package) macro fun add_liquidity_with_receipt_inner<$X, $Y, $Pool, $LP, $Receipt>(
        ...
        let (delta_l, delta_x, delta_y, receipt) = $add_liquidity_inner(pool_object, position.lp_position_mut());
        ...
        let info = add_liquidity_info_constructor(
            object::id(position), sqrt_pool_price_x64, delta_l, delta_x, delta_y
        );
    ...
    public(package) macro fun add_liquidity_with_receipt<$X, $Y, $Pool, $LP, $Receipt>(
        ...
        let (receipt, info) = add_liquidity_with_receipt_inner!(
            position, $config, $price_info, $debt_info, $pool_object, $add_liquidity_inner
        );
        if (info.delta_l() > 0) {
            info.emit();
        };
    ...
    public fun create_rebalance_receipt<X, Y, LP: store>(
        position: &mut Position<X, Y, LP>, config: &PositionConfig, ctx: &mut TxContext
    ...
    public(package) fun take_rebalance_fee<X, Y, LP, T>(
        position: &mut Position<X, Y, LP>, fee_bps: u16, balance: &mut Balance<T>, receipt: &mut RebalanceReceipt
    ) {
    ...
    public(package) macro fun rebalance_add_liquidity_with_receipt<$X, $Y, $Pool, $LP, $CetusReceipt>(
        ...
        let (cetus_receipt, info) = add_liquidity_with_receipt_inner!(
            position, $config, $price_info, $debt_info, $pool_object, $add_liquidity_lambda
        );
    ...
    public fun rebalance_repay_debt_x<X, Y, SX, LP: store>(
        position: &mut Position<X, Y, LP>, balance: &mut Balance<X>, receipt: &mut RebalanceReceipt,
        supply_pool: &mut SupplyPool<X, SX>, clock: &Clock
    ) {
        assert!(receipt.position_id == object::id(position), EPositionMismatch);

        let mut debt_shares = position.debt_bag.take_all();
        let (_, x_repaid) = supply_pool::repay_max_possible(supply_pool, &mut debt_shares, balance, clock);
        position.debt_bag.add<X, SX>(debt_shares);
    ...
    public fun rebalance_repay_debt_y<X, Y, SY, LP: store>(
        position: &mut Position<X, Y, LP>, balance: &mut Balance<Y>, receipt: &mut RebalanceReceipt,
    ...
    public fun consume_rebalance_receipt<X, Y, LP: store>(
        position: &mut Position<X, Y, LP>, receipt: RebalanceReceipt
    ) {
        assert!(receipt.position_id == object::id(position), EPositionMismatch);
        position.ticket_active = false;

        let RebalanceReceipt {
            id, position_id, delta_l, delta_x, delta_y, fees_taken, collected_amm_fee_x, collected_amm_fee_y,
            collected_amm_rewards, taken_cx, taken_cy, x_repaid, y_repaid, added_cx, added_cy, stashed_amm_rewards,
        } = receipt;
    ...
    public(package) macro fun calc_liquidate_col_x<$X, $Y, $LP>(
        ...
        model.calc_liquidate_col_x(
            p_x128, $max_repayment_amt_y, config.liq_margin_bps(), config.liq_bonus_bps(), config.base_liq_factor_bps(),
        )
    }
    ...
    public(package) macro fun calc_liquidate_col_y<$X, $Y, $LP>(
        ...
        model.calc_liquidate_col_y(
            p_x128, $max_repayment_amt_x, config.liq_margin_bps(), config.liq_bonus_bps(), config.base_liq_factor_bps(),
        )
    }
