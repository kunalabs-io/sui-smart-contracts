module kai_leverage::position_model_clmm {
    ...
    public fun x_by_liquidity_x64(
        self: &PositionModel, sqrt_p_x64: u128, delta_l: u128,
    ): u128 {
        ...
        // reach such a state, but in cases where we're doing some kind of a simulation it's possible in principle.
        // We handle this by limiting the result to `U128_MAX` (which is the max amount of token in existence).
        (util::min_u256(x_x64, (U128_MAX as u256)) as u128)
    }

    /// Calculate the amount of Y in the LP position for a given price and liquidity.
    /// Aborts if there's not enough liquidity in the position.
    public fun y_by_liquidity_x64(
        self: &PositionModel, sqrt_p_x64: u128, delta_l: u128,
    ): u128 {
        assert!(delta_l <= self.l, EInsufficientLiquidity);
        if (sqrt_p_x64 <= self.sqrt_pa_x64) {
    ...
    public fun margin_x64(
        self: &PositionModel, p_x128: u256
    ): u128 {
    ...
    /// Calculate the `l` by which the LP position must be reduced so that the margin level goes above the
    /// deleverage threshold after debt repayment w.r.t. the `base_deleverage_factor`. It assumes that
    /// conversion between X and Y is not done so `dx` is only repaid using available `cx` and the X amounts from
    /// the LP position and same for Y.
    /// 
    /// The `l` is calculated so that the position reaches a target margin level after the debt repayment.
    /// The target margin level is defined as the margin level that would be reached if `deleverage_factor_bps`
    /// of the debt is repaid at the moment margin falls below the deleverage threshold. This means that the returned
    /// `l` increases as the margin level decreases.
    /// 
    /// When extra collateral `cx` or `cy` is present, it is assumed that this will also be used to repay debt and together
    /// with returned `l` will amount to `deleverage_factor_bps` of debt repaid.
    /// 
    /// Summary:
    /// - when `M >= Md` returns 0
    /// - when `Md > M > 1` returns `l` such that the position reaches a constant target margin after the deleverage
    ...
    /// - if the target debt value cannot be repaid with position's total liquidity and extra collateral (`cx` and `cy`),
    ///   returns `position.l`
    public fun calc_max_deleverage_delta_l(
        position: &PositionModel, p_x128: u256, deleverage_margin_bps: u16, base_deleverage_factor_bps: u16,
    ): u128 {
        ...
        let (dx_x64, dy_x64, deleverage_factor_x64) = {
            ...
            let original_debt_val_x64 = (position.dx as u256) * (p_x64 as u256) + ((position.dy as u256) << 64); // 128.64

            let can_repay_x = u64::min(position.dx, position.cx);
            let can_repay_y = u64::min(position.dy, position.cy);
            let value_repaid_x64 = (can_repay_x as u256) * (p_x64 as u256) + ((can_repay_y as u256) << 64); // 128.64
        ...
        let to_deleverage_val_x64 = util::divide_and_round_up_u256(debt_val_x64 * deleverage_factor_x64, 1 << 64); // 128.64
    ...
    /// 
    /// The factor is calculated so that the position is above the liquidation threshold after the liquidation.
    /// The target margin level is one that would be reached if `base_liq_factor_bps` of the debt is repaid
    /// at the moment margin falls below the liquidation threshold. This means that the returned factor increases
    /// as the margin level decreases.
    /// 
    /// If the margin level is below the half-way point between the liquidation threshold and the critical margin level,
    /// the factor is 1. The critical margin level is defined as the margin level at which the position cannot be
    /// liquidated without incuring bad debt (while respecting the liquidation bonus, `Mc = 1 + liq_bonus`).
    /// 
    /// If the margin level is below the critical margin level, then the factor is calculated so that maximum possible
    /// of debt amount is liquidated while making sure there's enough collateral to cover the liquidation bonus.
    /// This means that as the current margin falls below the critical margin level, the factor decreases.
    /// 
    /// Summary:
    /// - when `M >= Ml` returns 0
    /// - when `Ml > M > (Ml + Mc) / 2` returns a factor so that the position reaches a constant target margin after liquidation
    /// - when `(Ml + Mc) / 2 >= M >= Mc` returns 1
    /// - when `Mc > M` returns a factor so that maximum possible debt is liquidated while respecting the liquidation bonus
    ///
    public fun calc_max_liq_factor_x64(
        current_margin_x64: u128, liq_margin_bps: u16, liq_bonus_bps: u16, base_liq_factor_bps: u16,
    ): u128 {
    ...
    /// Returns `true` if the position is "fully deleveraged". A position is considered fully deleveraged
    /// when all the liquidity has been withdrawn from the AMM pool and the debt that can be repaid directly
    /// (i.e. cx -> dx, cy -> dy) has been repaid.
    /// If this is true, then `dx > 0` implies `cx = 0` and `dy > 0` implies `cy = 0`. Also if `dx > 0 && dy > 0`
    /// then `cx == 0 && cy == 0`.
    ...
    public fun margin_below_threshold(position: &PositionModel, p_x128: u256, margin_threshold_bps: u16): bool {
    ...
    /// (up to given `max_repayment_amt_y`) and `reward_amt_x` is the amount of X returned to the liquidator.
    /// 
    /// Note:
    ...
    ///   - `cx == 0`
    /// - the position is liquidated so that the margin level is above the liquidation threshold after the liquidation
    ///   if possible for the the given `max_repayment_amt_y` and available collateral.
    /// - always respects the liquidation bonus, even if there's not enough collateral to cover a full liquidation
    /// - never aborts
    /// 
    /// See documentation for `calc_max_liq_factor_x64` for more details on how the liquidation factor is calculated.
    /// 
    public fun calc_liquidate_col_x(
        position: &PositionModel, p_x128: u256, max_repayment_amt_y: u64,
        liq_margin_bps: u16, liq_bonus_bps: u16, base_liq_factor_bps: u16,
    ): (u64, u64) {
        ...
        let margin_x64 = util::min_u256(
            (asset_value_x64 << 64) / debt_value_x64, U128_MAX as u256
        ); // 64.64
        ...
        let max_liq_factor_x64 = calc_max_liq_factor_x64(
            (margin_x64 as u128), liq_margin_bps, liq_bonus_bps, base_liq_factor_bps
        ) as u256; // 64.64
        ...
        let repayment_amt_y = util::min_u256(
            util::divide_and_round_up_u256(repayment_value_x64, 1 << 64),
            max_repayment_amt_y as u256
        ) as u64;
        let reward_amt_x = util::min_u256(
            util::divide_and_round_up_u256(repayment_value_with_bonus_x64 * (position.cx as u256), asset_value_x64),
            position.cx as u256
        ) as u64;
    ...
    /// (up to given `max_repayment_amt_x`) and `reward_amt_y` is the amount of Y returned to the liquidator.
    /// 
    /// Note:
    /// - returns `(0, 0)` when the position can't be liquidated:
    ///   - it's not below the liquidation threshold
    ...
    public fun calc_liquidate_col_y(
        position: &PositionModel, p_x128: u256, max_repayment_amt_x: u64,
        liq_margin_bps: u16, liq_bonus_bps: u16, base_liq_factor_bps: u16,
    ): (u64, u64) {
        if (!margin_below_threshold(position, p_x128, liq_margin_bps)) {
            return (0, 0)
        ...
        // calc repayment value
        let possible_repayment_factor_x64 = ((max_repayment_amt_x as u256) << 64) / (position.dx as u256); // 64.64
    ...
    fun test_calc_max_deleverage_delta_l() {
        ...
        let delta_l = calc_max_deleverage_delta_l(
            &position, p_x128, deleverage_margin_bps, 2000
        );
        assert!(delta_l == 0, 0);
        ...
        let p_x128: u256 = (15 << 128) / 10; // 1.5
        let delta_l = calc_max_deleverage_delta_l(
            &position, p_x128, deleverage_margin_bps, 10000
        );
        ...
        let delta_l =calc_max_deleverage_delta_l(
            &position, p_x128, deleverage_margin_bps, base_deleverage_factor_bps
        ); 
    ...
    fun test_calc_max_liq_factor() {
        ...
        let max_liq_factor_x64 = calc_max_liq_factor_x64(
            current_margin_x64, liq_margin_bps, liq_bonus_bps, base_liq_factor_bps
        );
    ...
    fun test_calc_liquidate_col_x() {
        ...
        let (repayment_amt_y, reward_amt_x) = calc_liquidate_col_x(
            &position, p_x128, max_repayment_amt_y, liq_margin_bps, liq_bonus_bps, base_liq_factor_bps
        );
    ...
    fun test_calc_liquidate_col_y() {
        ...
        let (repayment_amt_x, reward_amt_y) = calc_liquidate_col_y(
            &position, p_x128, max_repayment_amt_x, liq_margin_bps, liq_bonus_bps, base_liq_factor_bps
        );
