module kai_leverage::debt_info {
    ...
    public fun add_from_supply_pool<T, ST>(self: &mut DebtInfo, pool: &mut SupplyPool<T, ST>, clock: &Clock) {
    ...
    public fun calc_repay_by_shares(
        self: &ValidatedDebtInfo, `type`: TypeName, share_value_x64: u128
    ): u64 {
        calc_repay_lossy(self, `type`, share_value_x64)
    }

    /// Calculates the debt share amount required to repay the given amount of debt.
    public fun calc_repay_by_amount(
        self: &ValidatedDebtInfo, `type`: TypeName, amount: u64
    ): u128 {
