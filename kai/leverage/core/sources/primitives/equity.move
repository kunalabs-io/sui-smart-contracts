module kai_leverage::equity {
    ...
    /// Increase the underlying value and issue corresponding shares. Input value is in `UQ64.64` format.
    ...
    public fun increase_value_and_issue<T>(registry: &mut EquityRegistry<T>, value: u64): EquityShareBalance<T> {
    ...
    /// Returns the value redeemed (the amount the underlying value was reduced by).
    /// The returned value is rounded down and the fraction part is given back to the underlying, effectively
    ...
    /// the function also returns the calculated redeemed value (the amount the underlying value would be reduced by).
    /// This value is always greater than or equal to the required amount.
    /// Returns `(share_amount_x64, redeemed_value_x64)` tuple. The input and returned values are in `UQ64.64` format.
    public fun calc_redeem_for_amount_x64<T>(
        registry: &EquityRegistry<T>, amount_x64: u128
    ): (u128, u128) {
    ...
    /// Calculate the amount of `EquityShareBalance` required to redeem the given amount of underlying value when calling the
    ...
    public fun calc_redeem_for_amount<T>(
        registry: &EquityRegistry<T>, amount: u64
    ): u128 {
    ...
    /// Calculate the amount of share `Balance` required to redeem the given amount of underlying value when calling the
    /// `redeem_lossy` function.
    /// Since the redeemed value can sometimes be different from the required due to integer arithmetic,
    /// the function also returns the calculated redeemed value (the amount the underlying value would be reduced by).
    /// This value is always greater than or equal to the required amount.
    ...
    public fun calc_balance_redeem_for_amount<T>(
        ...
        let share_amount = (util::muldiv_round_up_u128(
            (amount as u128),
            registry.supply_x64,
            registry.underlying_value_x64
        ) as u64);
    ...
    /// Lossy. Converts the `EquityShareBalance` to a corresponding `Balance`. The fractional part is lost (if any)
    ...
    public fun into_balance_lossy<T>(share: EquityShareBalance<T>, treasury: &mut EquityTreasury<T>): Balance<T> {
        let EquityShareBalance { value_x64: share_value_x64 } = share;
    ...
    /// Convert a `EquityShareBalance` to a `Balance` while preserving the fractional part. Not lossy but doesn't
    /// consume all the shares.
    public fun into_balance<T>(share: &mut EquityShareBalance<T>, treasury: &mut EquityTreasury<T>): Balance<T> {
    ...
    public fun from_balance<T>(treasury: &mut EquityTreasury<T>, balance: Balance<T>): EquityShareBalance<T> {
    ...
    /// Split a `EquityShareBalance` and take a sub balance from it. Input amount is in `UQ64.64` format.
    public fun split_x64<T>(shares: &mut EquityShareBalance<T>, amount_x64: u128): EquityShareBalance<T> {
