module kai_leverage::debt {
    ...
    /// Increase the liability value and issue corresponding debt shares. Input value is in `UQ64.64` format.
    public fun increase_liability_and_issue_x64<T>(
        registry: &mut DebtRegistry<T>, value_x64: u128
    ): DebtShareBalance<T> {
    ...
    public fun increase_liability_and_issue<T>(registry: &mut DebtRegistry<T>, value: u64): DebtShareBalance<T> {
    ...
    /// Returns the value repaid (the amount the liability was reduced by).
    /// The repaid amount is rounded up and the fractional difference is reduced from the total liability,
    ...
    public fun repay_lossy<T>(registry: &mut DebtRegistry<T>, share: DebtShareBalance<T>): u64 {
        ...
        // due to the fact that the value is calculated by rounding up which may mean the fractional part
        // is larger that the total remaining liability. Use `repay_x64` to handle this edge case if needed.
        let fraction = if (value_x64 % Q64 == 0) { 0 } else { Q64 - (value_x64 % Q64) };
        registry.liability_value_x64 = registry.liability_value_x64 - fraction;

        value
    }

    /// Calculate the `EquityShareBalance` required to repay the given amount when calling the `repay_x64` function. 
    /// Since the resulting repaid value can sometimes be different from the required due to integer arithmetic,
    /// the function also returns the calculated repaid value (the amount the liability would be reduced by).
    /// This value is always lower than or equal to the required amount.
    /// Returns `(share_amount_x64, repaid_value_x64)` tuple. The input and return values are in `UQ64.64` format.
    public fun calc_repay_for_amount_x64<T>(
        registry: &DebtRegistry<T>, amount_x64: u128
    ): (u128, u128) {
    ...
    /// Calculate the `EquityShareBalance` required to repay the given amount when calling the `repay_lossy` function.
    ...
    public fun calc_repay_for_amount<T>(
        registry: &DebtRegistry<T>, amount: u64
    ): u128 {
        util::muldiv_u128(
            (amount as u128) * Q64,
            registry.supply_x64,
    ...
    /// Calculate the share `Balance` required to repay the given amount when calling the `repay_lossy` function.
    /// Since the resulting repaid value can sometimes be different from the required due to integer arithmetic,
    /// the function also returns the calculated repaid value (the amount the liability would be reduced by).
    /// This value is always lower than or equal to the required amount.
    public fun calc_balance_repay_for_amount<T>(
        ...
        let share_amount = (util::muldiv_u128(
            (amount as u128),
            registry.supply_x64,
            registry.liability_value_x64,
        ) as u64);
    ...
    /// Lossy. Converts the `DebtShareBalance` to a corresponding `Balance`. The fractional difference from rounding
    /// up is added to the total supply of shares which effectively reduces the debt of other shares against the total liability.
    public fun into_balance_lossy<T>(share: DebtShareBalance<T>, treasury: &mut DebtTreasury<T>): Balance<T> {
        let DebtShareBalance { value_x64: share_value_x64 } = share;
        ...
        let fraction = if (share_value_x64 % Q64 == 0) { 0 } else { Q64 - (share_value_x64 % Q64) };
        treasury.registry.supply_x64 = treasury.registry.supply_x64 + fraction;

        // the share supply can become larger than `(Q64 - 1) * Q64` and in this case not all shares can be converted
        // as the coin for the final `Q64 - 1` shares can't be minted due to u64 max on the coin supply, but this
        // edge case is not very important to support in practice
        coin::mint_balance(&mut treasury.cap, value)
    }

    /// Convert a `DebtShareBalance` to a `Balance` while preserving the fractional part. Not lossy but doesn't
    /// consume all the shares.
    public fun into_balance<T>(share: &mut DebtShareBalance<T>, treasury: &mut DebtTreasury<T>): Balance<T> {
    ...
    public fun from_balance<T>(treasury: &mut DebtTreasury<T>, balance: Balance<T>): DebtShareBalance<T> {
