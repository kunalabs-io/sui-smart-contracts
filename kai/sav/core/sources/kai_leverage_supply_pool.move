module kai::kai_leverage_supply_pool {
    use std::u64;
    use sui::clock::Clock;
    ...
    entry public(package) fun new<T, ST>(
        _supply_pool: &SupplyPool<T, ST>, ctx: &mut TxContext
    ) {
    ...
    public fun join_vault<T, ST, YT>(
        strategy: &mut Strategy<T, ST>, strategy_cap: &AdminCap,
        vault: &mut Vault<T, YT>, vault_cap: &VaultAdminCap<YT>,
        ctx: &mut TxContext
    ) {
    ...
    public fun remove_from_vault<T, ST, YT>(
        strategy: &mut Strategy<T, ST>, cap: &AdminCap,
        ...
        vault::new_strategy_removal_ticket(
            option::extract(&mut strategy.vault_access), redeemed_balance
        )
    }

    entry fun migrate<T, ST>(
        cap: &AdminCap, strategy: &mut Strategy<T, ST>
    ) {
        assert_admin(cap, strategy);
    ...
    public fun rebalance<T, ST, YT>(
        ...
        if (to_repay > 0) {
            ...
            let mut redeemed_balance_t = supply_pool.withdraw(
                strategy.shares.split(redeem_st_amt), clock
            );

            if (redeemed_balance_t.value() > to_repay) {
                let extra_amt = redeemed_balance_t.value() - to_repay;
                strategy.collected_profit_t.join(
                    redeemed_balance_t.split(extra_amt)
                );
            };
    ...
    fun skim_base_profits<T, ST>(
        strategy: &mut Strategy<T, ST>, supply_pool: &mut SupplyPool<T, ST>, clock: &Clock
    ) {
        ...
        if (share_value > strategy.underlying_nominal_value_t) {
            ...
            let redeemed_balance = supply_pool.withdraw(
                strategy.shares.split(redeem_share_amount), clock
            );
    ...
    public fun inject_incentives<T, ST>(
        strategy: &mut Strategy<T, ST>, balance: Balance<T>
    ) {
    ...
    public fun collect_and_hand_over_profit<T, ST, YT>(
        strategy: &mut Strategy<T, ST>, cap: &AdminCap, vault: &mut Vault<T, YT>,
        supply_pool: &mut SupplyPool<T, ST>,
        clock: &Clock
    ) {
        assert_admin(cap, strategy);
    ...
    public fun withdraw<T, ST, YT>(
        strategy: &mut Strategy<T, ST>, ticket: &mut WithdrawTicket<T, YT>,
        supply_pool: &mut SupplyPool<T, ST>,
        clock: &Clock
    ) {
        assert_version(strategy);
