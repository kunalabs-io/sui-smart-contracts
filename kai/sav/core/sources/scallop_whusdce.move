module kai::scallop_whusdce {
    ...
    const SCALLOP_POOL_ID: address = @0x4ace6648ddc64e646ba47a957c562c32c9599b3bba8f5ac1aadb2ae23a2f8ca0;
    const SCALLOP_MARKET_ID: address = @0xa757975255146dc9686aa823b7838b507f315d704f428cbadad2f4ea061939d9;
    const SCALLOP_REWARDS_POOL_ID: address = @0xf4268cc9b9413b9bfe09e8966b8de650494c9e5784bf0930759cfef4904daff8;
    ...
    entry fun join_vault(
        vault_cap: &VaultAdminCap<YWHUSDCE>, vault: &mut Vault<WHUSDCE, YWHUSDCE>,
    ...
    public fun remove_from_vault(
        ...
        let unstaked_susdc = scallop_pool::user::unstake(
            scallop_pool, &mut strategy.scallop_pool_acc, amount_susdc, clock, ctx
        );
    ...
    public fun rebalance(
        cap: &AdminCap, strategy: &mut Strategy, vault: &mut Vault<WHUSDCE, YWHUSDCE>,
        ...
        if (to_repay > 0) {
            ...
            let unstaked_susdc = scallop_pool::user::unstake(
                scallop_pool, &mut strategy.scallop_pool_acc, unstake_susdc_amt, clock, ctx
            );
            let redeemed_coin = scallop_protocol::redeem::redeem(
                scallop_version, scallop_market, unstaked_susdc, clock, ctx
            );
            ...
            strategy.underlying_nominal_value_usdc = strategy.underlying_nominal_value_usdc + borrow_amt;
        }
    }
    ...
    public fun skim_base_profits(
        ...
        let staked_amount_susdc = scallop_pool::spool_account::stake_amount(&strategy.scallop_pool_acc);
        let unstaked_susdc = scallop_pool::user::unstake(
            scallop_pool, &mut strategy.scallop_pool_acc, staked_amount_susdc, clock, ctx
        );
        let redeemed_coin = scallop_protocol::redeem::redeem(
            scallop_version, scallop_market, unstaked_susdc, clock, ctx
        );
        ...
        if (balance::value(&redeemed_balance_usdc) > strategy.underlying_nominal_value_usdc) {
            let profit_amt = balance::value(&redeemed_balance_usdc) - strategy.underlying_nominal_value_usdc;
            balance::join(
                &mut strategy.collected_profit_usdc, 
    ...
    public fun deposit_sold_profits(
        cap: &AdminCap, strategy: &mut Strategy,
        vault: &mut Vault<WHUSDCE, YWHUSDCE>, mut profit: Balance<WHUSDCE>,
        clock: &Clock
    ) {
        assert_admin(cap, strategy);
        assert_version(strategy);
    ...
    public fun withdraw(
        strategy: &mut Strategy, ticket: &mut WithdrawTicket<WHUSDCE, YWHUSDCE>,
        scallop_version: &ScallopVersion, scallop_market: &mut ScallopMarket, scallop_pool: &mut ScallopPool,
        clock: &Clock, ctx: &mut TxContext
    ) {
        assert_version(strategy);
