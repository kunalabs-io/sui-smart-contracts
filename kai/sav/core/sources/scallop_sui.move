...
/// NOTE: This module has been deprecated due to `kai_sav::ysui::YSUI` not initializing
/// during upgrade and thus the relevant `Vault` not being able to be initialized.
/// The new module is `kai_sav::scallop_sui_proper`.
...
public fun rebalance(
    ...
    if (to_repay > 0) {
        let staked_amount_ssui = scallop_pool::spool_account::stake_amount(
...
public fun withdraw(
    strategy: &mut Strategy,
    ticket: &mut WithdrawTicket<SUI, YSUI>,
    scallop_version: &ScallopVersion,
    scallop_market: &mut ScallopMarket,
    scallop_pool: &mut ScallopPool,
    clock: &Clock,
