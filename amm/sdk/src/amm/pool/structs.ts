...
export class Pool {
  ...
  static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<Pool> {
    ...
    return this.fromBcs(typeArgs as [Type, Type], (res as any).data!.bcs!.bcsBytes, 'base64')
  }
}
...
export class PoolRegistry {
  ...
  static async fetch(provider: JsonRpcProvider, id: ObjectId): Promise<PoolRegistry> {
    ...
    if (res.error) {
      throw new Error(`Error fetching PoolRegistry object at ${id}: ${(res as any).error.tag}`)
    }
