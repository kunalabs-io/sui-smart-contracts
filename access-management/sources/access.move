module access_management::access {
    ...
    public fun migrate_policy_version(
        policy: &mut Policy, admin: &PackageAdmin
    ) {
    ...
    public fun add_empty_rule(
        policy: &mut Policy, admin: &PackageAdmin, ctx: &mut TxContext,
    ): address {
        check_version(policy);
        assert!(policy.package == admin.package, ENotPolicyAdmin);
    ...
    public fun drop_rule(
        policy: &mut Policy, admin: &PackageAdmin, rule_id: address
    ) {
        check_version(policy);
        assert!(policy.package == admin.package, ENotPolicyAdmin);
    ...
    public fun add_condition_to_rule_with_config<Condition: drop, Config: store + copy + drop>(
        policy: &mut Policy, admin: &PackageAdmin, rule_id: address, config: Config
    ) {
        check_version(policy);
        assert!(policy.package == admin.package, ENotPolicyAdmin);
    ...
    public fun add_condition_to_rule<Condition: drop>(
        ...
        add_condition_to_rule_with_config<Condition, ConfigNone>(
            policy, admin, rule_id, ConfigNone {}
        );
    }
    ...
    public fun new_request_for_resource<T: key>(
        action: String, resource: &T, ctx: &mut TxContext,
    ): ActionRequest {
    ...
    public fun new_request_with_context<Action: drop>(
        _: Action, context: DynamicMap<std::ascii::String>
    ): ActionRequest {
        ActionRequest {
            action_name: type_name::get<Action>(),
            context,
    ...
    public fun get_condition_witness<
        Condition,
        Action: drop,
        Config: store + copy + drop
    >(
        policy: &Policy, entity: &Entity, rule_id: address
    ): ConditionWitness<Condition, Config> {
        check_version(policy);
        assert!(policy.enabled, EPolicyDisabled);
        assert!(
            policy.allowed_entities.contains(&object::id(entity)), EEntityNotAllowed
        );
    ...
    public fun approve_condition<Condition: drop, Config: store + copy + drop>(
        request: &mut ActionRequest, witness: &ConditionWitness<Condition, Config>, _: Condition
    ) {
        let condition_name = type_name::get<Condition>();
        request.approved_conditions.insert(condition_name, witness.rule_id);
    }

    /// Aborts with `EInvalidConditionApproval` if the approved condition's rule id does not match the requested.
    /// Aborts with `std::vector::EKeyDoesNotExist` if the condition is not needed for the action request or
    /// it has already been approved.
    /// Aborts with `sui:vec_map::EMapEmpty` if one or multiple required conditions have not been approved.
    public fun approve_and_return_context(
        request: ActionRequest, entity: &Entity, policy: &Policy, rule_id: address
    ): DynamicMap<String> {
        check_version(policy);
        assert!(policy.enabled, EPolicyDisabled);
        assert!(
            policy.allowed_entities.contains(&object::id(entity)),
            EEntityNotAllowed
        );
        let rule = &policy.rules[&rule_id];
    ...
    public fun approve_request(request: ActionRequest, entity: &Entity, policy: &Policy, rule_id: address) {
        check_version(policy);
