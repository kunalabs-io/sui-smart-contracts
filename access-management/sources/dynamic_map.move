...
module access_management::dynamic_map {
    ...
    /// that key `k: K`.
    public fun insert<K: copy + drop + store, V: copy + drop + store>(map: &mut DynamicMap<K>, k: K, v: V) {
    ...
    /// Aborts with `sui::dynamic_field::EFieldDoesNotExist` if the map does not have an entry with
    /// that key `k: K`.
    public fun borrow_mut<K: copy + drop + store, V: copy + drop + store>(map: &mut DynamicMap<K>, k: K): &mut V {
    ...
    /// Aborts with `sui::dynamic_field::EFieldDoesNotExist` if the map does not have an entry with
    /// that key `k: K`.
    public fun remove<K: copy + drop + store, V: copy + drop + store>(map: &mut DynamicMap<K>, k: K): V {
