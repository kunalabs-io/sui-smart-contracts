...
public fun test_deposit_and_withdraw() {
    ...
    assert_and_destroy_balance(
        pool::collect_rewards_direct<BAR, SHARES>(&mut pool, &mut stake, 200),
        200,
    );
...
public fun test_merge_stakes() {
    ...
    let stake3 = pool::deposit_shares_new(
        &mut pool,
        balance::create_for_testing<SHARES>(0),
        ticket,
        ctx,
    );
    ...
    // clean up
    assert_and_destroy_balance(
        pool::collect_rewards_direct<FOO, SHARES>(&mut pool, &mut stake1, 300),
        300,
    );
...
public fun test_collect_rewards() {
    ...
    let mut stake2 = pool::deposit_shares_new(
        &mut pool,
        balance::create_for_testing(300),
        ticket,
        ctx,
    );
...
public fun test_remove_from_farm() {
    ...
    let ticket = pool::new_top_up_ticket(&mut pool);
    assert_and_destroy_balance(
        pool::withdraw_shares(&mut pool, &mut stake, 100, ticket),
        100,
    );
    pool::destroy_empty_stake(stake);
