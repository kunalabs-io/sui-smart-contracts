...
fun assert_values<T, K: copy>(
    td: &TimeDistributor<T, K>,
    total_weight: u64,
    unlocked_balance: u64,
    update_ts: u64,
) {
    let (act_total_weight, act_unlocked_balance, act_update_ts) = td::get_values(td);
    assert!(total_weight == act_total_weight);
    assert!(unlocked_balance == act_unlocked_balance);
    assert!(update_ts == act_update_ts);
}
...
public fun test_no_balance() {
    ...
    assert_and_destroy_balance(
        td::member_withdraw_all_by_idx(&mut td, 0, &clock),
        0,
    );
...
public fun test_with_two_members() {
    ...
    assert_and_destroy_balance(
        td::member_withdraw_all_by_idx(&mut td, 1, &clock),
        0,
    );
...
public fun test_change_unlock_per_second() {
    ...
    assert_and_destroy_balance(
        td::member_withdraw_all(&mut td, &0, &clock),
        999,
    );
